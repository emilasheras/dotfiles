...
# ubuntu-bootstrap.sh — v7
# Workstation bootstrap:
# 1) preflight
# 2) core apt prerequisites
# 3) CLI baseline
# 4) fonts
# 5) GUI apps installation
# 6) verification
...
#   2) JetBrains Mono Nerd Font v3.4.0 — pinned release from the official
#      nerd-fonts project, extracted to /usr/local/share/fonts/.
#   3) GNOME font defaults — system-wide dconf defaults so new GNOME sessions
#      prefer JetBrains Mono as the interface and monospace font.
#
# Idempotency:
#   - skip if fontconfig already sees the font family
#   - refresh fc-cache after installation
#   - write the same dconf keyfile on every run
# -----------------------------------------------------------------------------
...
JETBRAINS_NERD_FONT_FAMILY="JetBrainsMono Nerd Font"
JETBRAINS_NERD_FONT_MONO_FAMILY="JetBrainsMono Nerd Font Mono"
JETBRAINS_SYSTEM_FONT_SIZE="11"

configure_system_font_defaults() {
  info "Configuring GNOME system font defaults"

  local dconf_profile="/etc/dconf/profile/user"
  local dconf_db_dir="/etc/dconf/db/local.d"
  local dconf_keyfile="${dconf_db_dir}/10-jetbrains-fonts"

  sudo install -d -m 0755 /etc/dconf/profile

  if [[ ! -f "$dconf_profile" ]]; then
    info "Creating dconf profile for system defaults"
    cat <<'EOF' | sudo tee "$dconf_profile" >/dev/null
user-db:user
system-db:local
EOF
  else
    sudo grep -qxF 'user-db:user' "$dconf_profile" || echo 'user-db:user' | sudo tee -a "$dconf_profile" >/dev/null
    sudo grep -qxF 'system-db:local' "$dconf_profile" || echo 'system-db:local' | sudo tee -a "$dconf_profile" >/dev/null
  fi

  sudo install -d -m 0755 "$dconf_db_dir"

  cat <<EOF | sudo tee "$dconf_keyfile" >/dev/null
[org/gnome/desktop/interface]
font-name='${JETBRAINS_NERD_FONT_FAMILY} ${JETBRAINS_SYSTEM_FONT_SIZE}'
monospace-font-name='${JETBRAINS_NERD_FONT_MONO_FAMILY} ${JETBRAINS_SYSTEM_FONT_SIZE}'
EOF

  info "Updating dconf databases"
  sudo dconf update

  ok "GNOME font defaults written"
}
...
install_fonts() {
  ...
  if [[ -d "$JETBRAINS_NERD_FONT_DIR" ]] && fc-list | grep -qi "$JETBRAINS_NERD_FONT_FAMILY"; then
...
install_ghostty() {
  if require_cmd ghostty; then
    ok "Ghostty already installed"
    return 0
  fi

  info "Installing Ghostty from the Ubuntu repositories"
  sudo DEBIAN_FRONTEND=noninteractive apt-get install -y ghostty
}

# -----------------------------------------------------------------------------
# GUI apps installation (continued)
# -----------------------------------------------------------------------------
...
verify_all() {
  ...
  _check "fontconfig cache tool"        command -v fc-cache
  _check "dconf CLI"                   command -v dconf
  _check "JetBrains Mono Nerd Font"     bash -c 'fc-list | grep -qi "JetBrainsMono Nerd Font"'
  ...
  _check "Ghostty"                     command -v ghostty
