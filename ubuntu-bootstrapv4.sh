...
#
# FIX v4: switched away from `snap install obsidian --classic`.
# Obsidian via Snap fails to launch on Ubuntu 26.04 / Wayland because the
# Electron app cannot initialise the display session under classic confinement.
# The official .deb ships a proper .desktop file and works correctly.
# -----------------------------------------------------------------------------
...
# FIX v4: added two post-install steps that were missing:
#   1. `systemctl enable --now tailscaled`  — ensures the daemon is up before
#      the operator command runs (race condition on fresh install).
#   2. `tailscale set --operator=$USER`     — grants the current user
#      permission to run tailscale/systray without sudo, which is required for
#      the systray app to appear and function from the GNOME app launcher.
install_tailscale() {
  ...
  info "Setting Tailscale operator permission for ${USER}"
  if sudo tailscale set --operator="$USER"; then
    ok "Tailscale operator set for ${USER}"
  else
...
verify_all() {
  ...
  _check "Document Viewer"      bash -c 'dpkg -s evince || command -v evince'
  ...
  if ((fail > 0)); then
    warn "Some apps were not detected — review the log: ${LOG_FILE}"
  else
